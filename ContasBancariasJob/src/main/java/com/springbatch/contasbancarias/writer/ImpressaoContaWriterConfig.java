...
public class ImpressaoContaWriterConfig {
	@Bean
	public JdbcBatchItemWriter<Conta> jdbcContaWriter(@Qualifier("appDataSource")
	    DataSource dataSource) {
		return new JdbcBatchItemWriterBuilder<Conta>().
				   dataSource(dataSource)
				   .sql("INSERT INTO conta (tipo,limite,cliente_id) values(?,?,?)")
				   .itemPreparedStatementSetter(itemPreparedeStatmentSetter())
				   .build();
	}

	private ItemPreparedStatementSetter<Conta> itemPreparedeStatmentSetter() {
